...
### 5. RJ Bingo (Flask + HTMX)

**Local Path:** `C:\claude_projects\rjbingo`
**Remote Path:** `/home/ec2-user/rjbingo/`
**Service:** systemd (`rjbingo.service`)

**Key Features:**
- Dio/Rainbow/Black Sabbath lyrics bingo card generator
- Print-and-play stateless game (no user data)
- HTMX-powered dynamic UI
- Read-only database (seeded by `init_db.py`)

**Deployment:**
```bash
python deploy.py rjbingo --yes
```

**Database:** Read-only, regenerated by `python3 init_db.py` on server if needed.
...
| rjbingo | `/home/ec2-user/rjbingo/data/bingo.db` |
